...
import { addExerciseToRoutine } from '../userFirestore';
...
const AddExerciseScreen = ({ route, navigation }) => {
  ...
  const [sets, setSets] = useState([{ kgs: '', reps: '', sets: '', rest: 60 }]);
  ...
  const handleSave = async () => {
    ...
    try {
      await addExerciseToRoutine(userId, routineId, exercise, sets);
      Alert.alert('Success', 'Exercise added to routine successfully!');
      navigation.goBack();
    } catch (error) {
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      await addExerciseToRoutine(userId, routineId, exercise, updatedSets);
      Alert.alert('Success', 'Exercise added to routine successfully!');
      navigation.goBack();
    } catch (error) {
      Alert.alert('Error', 'Failed to add exercise to routine.');
